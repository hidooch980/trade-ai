...
class TradeExecutor:
    ...
    async def execute(
        ...
        if broker_name.lower() not in broker_connection_manager.connected_brokers():
            return {
                "executed": False,
                "reason": "BROKER_DISCONNECTED"
            }

        health = await broker_connection_manager.health_check(broker_name)
