...
from app.chart_ai.smart_money.entry_score import SmartMoneyEntryScore
...
order_block = OrderBlockEngine().analyze(candles)

fvg = FVGEngine().analyze(candles)


result = SmartMoneyEntryScore().calculate(
    structure,
    liquidity,
    order_block,
    fvg
