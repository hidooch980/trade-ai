from datetime import datetime, timezone

from fastapi import APIRouter, Depends, HTTPException, Request, status
from sqlalchemy.ext.asyncio import AsyncSession

from app.db.session import get_db
from app.schemas.auth import (
    LoginRequest,
    RegisterRequest,
    RefreshRequest,
    LogoutRequest,
    TokenResponse,
    UserResponse,
    SessionResponse,
    ForgotPasswordRequest,
    ResetPasswordRequest,
)
from app.security.auth.jwt import create_access_token
from app.security.auth.dependencies import get_current_user
from app.security.auth.session_manager import (
    AuthSessionManager,
    RefreshTokenReplayError,
)
from app.repositories.auth_session_repository import AuthSessionRepository
from app.repositories.password_reset_repository import PasswordResetRepository
from app.services.user_service import AuthOutcome, UserService
from app.i18n.manager import i18n, normalize_language
from app.security.auth.rate_limit import auth_rate_limiter
from app.security.auth.password_reset import (
    generate_reset_token,
    hash_reset_token,
    reset_token_expiry,
)
from app.security.password import hash_password

router = APIRouter(prefix="/auth", tags=["auth"])


def get_session_manager(db: AsyncSession) -> AuthSessionManager:
    return AuthSessionManager(AuthSessionRepository(db))


@router.post(
    "/register",
    response_model=UserResponse,
    status_code=status.HTTP_201_CREATED,
)
async def register(
    data: RegisterRequest,
    db: AsyncSession = Depends(get_db),
):
    service = UserService(db)

    try:
        user = await service.create(
            email=str(data.email),
            username=data.username,
            password=data.password,
            language=normalize_language(data.language),
        )

        await db.commit()
        return user

    except ValueError as exc:
        await db.rollback()

        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=i18n.translate(
                "auth.email_exists"
                if "email" in str(exc).lower()
                else "auth.username_exists"
            ),
        )


@router.post("/login", response_model=TokenResponse)
async def login(
    data: LoginRequest,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    service = UserService(db)

    client_ip = (
        request.client.host
        if request.client
        else "unknown"
    )

    rate_key = f"login:{client_ip}:{data.username.lower()}"

    if not auth_rate_limiter.check(
        rate_key,
        limit=5,
        window_seconds=60,
    ):
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Too many authentication attempts",
        )

    i18n.set_language(
        normalize_language(data.language)
    )

    attempt = await service.authenticate(
        username=data.username,
        password=data.password,
    )

    if attempt.outcome is AuthOutcome.LOCKED:
        raise HTTPException(
            status_code=status.HTTP_423_LOCKED,
            detail="Account temporarily locked",
        )

    if not attempt.ok:
        # The attempt counter was just incremented on the user row. Without
        # this commit the 401 discards it and the lockout never arms.
        await db.commit()

        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=i18n.translate(
                "auth.invalid_credentials"
            ),
        )

    user = attempt.user

    access_token = create_access_token(
        user_id=user.id,
        username=user.username,
        language=user.language,
        role=user.role,
    )

    manager = get_session_manager(db)

    refresh_token, _ = await manager.create_session(
        user_id=user.id,
        device_name=data.device_name,
        ip_address=(
            request.client.host
            if request.client
            else None
        ),
        user_agent=request.headers.get("user-agent"),
    )

    await db.commit()

    return TokenResponse(
        access_token=access_token,
        refresh_token=refresh_token,
        token_type="bearer",
        user=user,
    )


@router.post("/refresh", response_model=TokenResponse)
async def refresh(
    data: RefreshRequest,
    request: Request,
    db: AsyncSession = Depends(get_db),
):
    manager = get_session_manager(db)

    try:
        new_refresh_token, session, replay_detected = await manager.rotate(
            data.refresh_token,
            ip_address=(
                request.client.host
                if request.client
                else None
            ),
            user_agent=request.headers.get("user-agent"),
        )

    except RefreshTokenReplayError:
        await db.commit()

        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Refresh token reuse detected",
        )

    if new_refresh_token is None or session is None:
        await db.rollback()

        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=i18n.translate(
                "auth.invalid_or_expired_session"
            ),
        )

    if replay_detected:
        await db.commit()
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Refresh token reuse detected",
        )

    user = await UserService(db).get_by_id(
        session.user_id
    )

    if not user:
        await db.rollback()

        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=i18n.translate(
                "auth.user_not_found"
            ),
        )

    access_token = create_access_token(
        user_id=user.id,
        username=user.username,
        language=user.language,
        role=user.role,
    )

    i18n.set_language(
        normalize_language(user.language)
    )

    await db.commit()

    return TokenResponse(
        access_token=access_token,
        refresh_token=new_refresh_token,
        token_type="bearer",
        user=user,
    )


@router.post("/logout")
async def logout(
    data: LogoutRequest,
    db: AsyncSession = Depends(get_db),
):
    manager = get_session_manager(db)

    await manager.revoke(
        data.refresh_token
    )

    await db.commit()

    return {"success": True}


@router.post("/logout-all")
async def logout_all(
    current_user=Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    repository = AuthSessionRepository(db)

    await repository.revoke_all_for_user(
        current_user.id
    )

    await db.commit()

    return {"success": True}


@router.get(
    "/sessions",
    response_model=list[SessionResponse],
)
async def sessions(
    current_user=Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    repository = AuthSessionRepository(db)

    return await repository.list_for_user(
        current_user.id
    )


@router.get(
    "/me",
    response_model=UserResponse,
)
async def me(
    current_user=Depends(get_current_user),
):
    i18n.set_language(
        normalize_language(current_user.language)
    )

    return current_user


@router.post("/forgot-password")
async def forgot_password(
    data: ForgotPasswordRequest,
    db: AsyncSession = Depends(get_db),
):
    service = UserService(db)

    user = await service.get_by_email(str(data.email))

    # پاسخ عمداً عمومی است تا وجود یا عدم وجود حساب افشا نشود.
    if not user:
        return {
            "success": True,
            "message": "If the account exists, a password reset token has been generated.",
        }

    repository = PasswordResetRepository(db)

    await repository.invalidate_user_tokens(user.id)

    raw_token = generate_reset_token()

    await repository.create(
        user_id=user.id,
        token_hash=hash_reset_token(raw_token),
        expires_at=reset_token_expiry(),
    )

    await db.commit()

    # در production این token باید از طریق Email/SMS provider
    # ارسال شود و نباید در response برگردانده شود.
    return {
        "success": True,
        "message": "If the account exists, a password reset token has been generated.",
    }


@router.post("/reset-password")
async def reset_password(
    data: ResetPasswordRequest,
    db: AsyncSession = Depends(get_db),
):
    repository = PasswordResetRepository(db)

    item = await repository.get_by_hash(
        hash_reset_token(data.token)
    )

    if not item:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=i18n.translate("auth.invalid_reset_token"),
        )

    now = datetime.now(timezone.utc)

    if item.used_at is not None or item.expires_at <= now:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=i18n.translate("auth.invalid_reset_token"),
        )

    user = await UserService(db).get_by_id(item.user_id)

    if not user:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=i18n.translate("auth.invalid_reset_token"),
        )

    user.password_hash = hash_password(data.new_password)
    user.failed_login_attempts = 0
    user.locked_until = None

    item.used_at = now

    # با تغییر Password تمام دستگاه‌ها خارج می‌شوند.
    await AuthSessionRepository(db).revoke_all_for_user(user.id)

    await db.commit()

    return {
        "success": True,
        "message": "Password reset successful.",
    }
